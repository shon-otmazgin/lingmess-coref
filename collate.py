...
class BertLikeCollator:
    ...
    def __call__(self, batch):
        ...
        if len(input_ids[0]) > 1 and len(input_ids[0][-1]) < self.max_segment_len:
            ...
            # remove the leftovers for the main batch
...
class DynamicBatchSampler:
    def __init__(self, dataset, collator, max_tokens_in_batch, max_segment_len, max_doc_len=None):
        self.max_tokens = max_tokens_in_batch
    ...
    def __iter__(self):
        ...
        for example in self.dataset:
            if self.max_doc_len is not None and example['length'] > self.max_doc_len:
                logger.warning(f'Skipping example with len of {example["length"]}. max_doc_len is {self.max_doc_len}')
                continue
            if not batch:
